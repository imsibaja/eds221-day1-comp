...
# Pipe operator: clean_names(toolik_weather) == toolik_weather %>% clean_names() == toolik_weather | clean_names()
# pi %>% sin
# [1] 1.224606e-16
# pi %>% sin %>% cos
# [1] 1
...
```{r}
ggplot(toolik_chlorophyll, aes(depth, active_chl_a_ug_l)) +
  geom_point(aes(colour = active_chl_a_ug_l), 
             show.legend = FALSE)+
  scale_color_gradient(low = "blue", high = "orange") +
  theme_minimal() +
  labs(title = "Toolik Chlorophyll Concentrations",
       x = "Lake Depth (m)",
       y = " Active Chlorophyll Concentration (µg/L)")
